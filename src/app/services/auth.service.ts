...
export class AuthService {
  ...
  async checkEmailExists(email: string): Promise<boolean> {
    try {
      const users = await this.db.getAllUsers(); // Asegúrate de que este método esté implementado en DatabaseService
      return users.some(user => user.email === email);
    } catch (error) {
      showAlertError('AuthService.checkEmailExists', error);
      return false;
    }
  }
  ...
  async register(user: User): Promise<boolean> {
    try {
      ...
      await this.db.saveUser(user); // Guarda el usuario en la base de datos
